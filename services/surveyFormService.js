...
const SURVEY_FORM_PATH = (process.env.SURVEY_FORM_LINK_PATH || "survey-form").replace(/^\/+|\/+$/g, "");
...
async function issueSurveyFormForApplication(application, userOverride = null) {
  const userId = typeof application.userId === "object" ? application.userId._id || application.userId : application.userId;
  const user =
    userOverride ||
