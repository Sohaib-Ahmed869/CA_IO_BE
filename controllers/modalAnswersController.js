...
const modalAnswersController = {
  ...
  getByCertification: async (req, res) => {
    try {
      ...
      const ma = await ModalAnswers.findOne({ certificationId: certId, isActive: true }).populate('certificationId', 'name');
      if (!ma) return res.status(404).json({ success: false, message: "Modal answers not found" });
      const response = ma.toObject();
      response.certificationName = ma.certificationId ? ma.certificationId.name : undefined;
